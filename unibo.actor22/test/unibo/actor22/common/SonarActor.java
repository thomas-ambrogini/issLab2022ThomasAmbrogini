	...
	protected void elabRequest(IApplMessage msg) {
		...
		switch( msgReq ) {
			...
			case ApplData.reqGetDistance :{
				...
				IApplMessage reply = MsgUtil.buildReply(getName(), ApplData.reqGetDistance, ""+d, msg.msgSender());
				ColorsOut.out( getName()  + " | reply= " + reply, ColorsOut.CYAN);
				sendReply(msg, reply );				
				break;
			}
			case ApplData.cmdUpdate      :{
				IDistance d = sonar.getDistance();
				IApplMessage reply = MsgUtil.buildReply(getName(), ApplData.cmdUpdate, ""+d, msg.msgSender());
				ColorsOut.out( getName()  + " | reply= " + reply, ColorsOut.CYAN);
				sendReply(msg, reply );				
				break;
			}
